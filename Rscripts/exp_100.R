...
#if (! ("mycon" %in% ls())) {
mycon = gzcon(gzfile(paste(dir, "refined_buggy_traces.csv.gz", sep="/"), open="r"))  
...
BigramTokenizer <- function(x) {RWeka::NGramTokenizer(x, RWeka::Weka_control(min = 1, max = 1, delimiters="  "))}
...
#r_evs_dm_df["program"] = sample_programs
